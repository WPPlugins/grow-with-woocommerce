...
/**
 * Prepares and displays Year on Year card data
 *
 * @since  1.1
 ...
 */
class WooCommerce_Grow_Card_Dashboard_Year_On_Year extends WooCommerce_Grow_Card_Dashboard {
	...
	public function set_target_month( $month, $year ) {
		if ( '' === $this->target_month ) {
			$this->target_month = $this->get_month_minus_one_year( $month, $year );
		}
	...
	public function set_target_year( $month, $year ) {
		if ( '' === $this->target_year ) {
			$this->target_year = $this->get_year_minus_one_year( $month, $year );
		}
	...
	/**
	 * Gets the comparison month for Year on Year data.
	 *
	 * @since 1.1
	 *
	 * @param $month
	 ...
	 * @return bool|string
	 */
	public function get_month_minus_one_year( $month, $year ) {
		return date( 'm', mktime( 0, 0, 0, $month, 1, $year - 1 ) );
	}
	 ...
	 * @return bool|string
	 */
	public function get_year_minus_one_year( $month, $year ) {
		return date( 'Y', mktime( 0, 0, 0, $month, 1, $year - 1 ) );
	}
}